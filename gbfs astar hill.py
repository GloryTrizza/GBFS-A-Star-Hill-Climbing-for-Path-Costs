...
class Agent(Environment):
    ...
    def A_star(graph, start, goal):
        ...
        while True:
            ...
            for i in neighbour.difference(p): # to go through all neighbours
                ...
                h[i] = Agent.get_h(i, goal) + Agent.get_cost(i)

            # best heuristics from list, then pick first item
            sorted_h = sorted(h.items(), key = operator.itemgetter(1))
            x = next(iter(sorted_h[0])) # first item in sorted list at this point contains the best heuristics
            p.append(x)
            if x == goal:
    ...
    def HCS(graph, start, goal):
        ...
        while True:
            neighbour = graph[start]  # what is next to the start position
            h = {}
            for i in neighbour.difference(p):  # to go through all neighbours except visited
                l = []
                l.append(str(start))
                l.append(str(i))  # the neighbour
                h[i] = Agent.get_cost(l)  # calc heuristics
                if least_cost == 0:
                    least_cost = h[i]
            ...
            # if it is the smallest, go ahead and append.
            if int(x) < least_cost:
                p.append(x)
                least_cost = int(Environment.cost[str(l)])
            # if it isnt the smallest, break
            else:
                break
    ...
    def __init__(self, Environment):
        ...
        print('Hill climb', Agent.HCS(Environment.myGraph, Environment.start, Environment.goal))
